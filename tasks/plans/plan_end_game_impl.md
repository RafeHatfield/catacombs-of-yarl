...
**Next step — TASK-011 (balance pass):** Guardian + Debt stats are placeholder. Needs harness Guardian-soak scenarios at varying record states to tune the wall's height.
...
**Direction (2026-06-01): content leads, balance held.** Rafe+Claude-Chat draft the audit dialogue first (the spine), then per-Guardian rise text (per tier), ally-fallback lines, six ending texts, Hollowmark through the Weighing — written against the locked rise order, anchors, and the orchestration events (`GuardianRoseEvent`/`AlliesFellBackEvent`/`DebtRoseEvent`/`WeighingResolvedEvent`). TASK-011 balance pass is HELD until content lands, so Guardian-soak tunes against the real narration rhythm (audit text length affects fight pacing) rather than silent placeholders. The soak-harness scaffolding can be roughed anytime on request; not pre-built to avoid tuning against placeholder rhythm.
