...
def write_theme(setname, code_index):
    ...
    text = text.replace(
        ...
        "# GENERATED by tools/floor_remediation/capture_floors.py - do not hand-edit.\n"
        "# Walls: the composition spike's `before` arm, UNCHANGED and held constant.\n"
        "# Floor: %s/%s (tile %d) in every floor slot. The floor is the only variable."
        % (setname, CODES[code_index], tid))
    name = "tile_themes_%s_%s.yaml" % (setname, CODES[code_index])
