...
| `../RD_CONVENTIONS.md`'s style ruling | `rd_plus__low_res` was chosen because it "produces the closest match to Oryx library aesthetics". That criterion no longer exists. The custom style `user__oryx_16_bit_fantasy_style_d970121a` is Oryx-trained and may no longer exist on the account at all. |
