...
## ROUND 3 — #207 the X-frame (branch `art/xframe-207`, stacked on the Λ's)
### The judge proved it can fail first (Ruling 47, `tools/cast_shadows/evidence/xframe/judge_control_walked.json`)
`cold_name_object.py` — one object, located by the seat's own WHERE inside the X's cell box,
≥ 4 of 5 with an accept word and no refuse word — on the **walked frame**: 2 of 5 named it, three
refused it (*"crossed planks — broken barricade / debris"*). FAIL, as Rafe's eye said.
### Hypothesis tested: the X has no height — it lies in the gap instead of standing in it
| candidate | what | seats | words |
|---|---|---|---|
| walked X | crossed baulks lying, bar behind | 2/5 (3 refused) | "crossed broken planks … debris" |
| **c1** | crossed STAKES stood on end, planted feet, crossing lashed wood-dark | **4/5** | "two crossed wooden planks (an x-shaped barricade)" — no *broken*, no *debris* |
| **c2** | the same, taller than wide, a low bar spanning the gap edge to edge | **5/5** | "wooden sawhorse / trestle barricade", "x-frame barricade" |
Standing is what moved the read: the refusals vanished the moment the members stood. **c2 lands**
(ids 9810/9811, seed 1338, hold 0.984): it names 5 of 5 and it holds the line — c1 stands in the
middle of a two-cell gap with room either side. The hedge Rafe's eye will meet: every c2 seat
reached for *sawhorse / trestle* (the low bar between the feet), where c1's reached for *crossed
planks*. Neither word is a refusal; the walk decides. c1's generation is kept beside the evidence.
The lashing is wood-dark (binding slot #208-blocked) — flagged to the palette lock, not solved.
Generated by `barricade_round3.py` (3 seeds each, pick by silhouette IoU, 2:1 landing); scene
guards unchanged; fast suite 2564 green.
