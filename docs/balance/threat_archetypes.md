...
|---|---|---|
| **Baseline (durable)** | orc, orc_brute, orc_veteran, orc_scout, skeleton, zombie, *cultist_blademaster* | brute = high-end durable beatstick (tier-defining, never nerf). **skeleton is a *textured* baseline** — has `skeleton` AI + ShieldWall component + bludgeoning-vulnerability (a counter-puzzle, bring a club/mace). cultist_blademaster = de facto **hot baseline**, see §6 Q1. |
| **Intrinsic spike** | lich (also escalator), wraith, troll/troll_ancient, *giant_spider* | **giant_spider is spike-lite** — mechanically identical to cave_spider (same on-hit poison framework); its "spike" is just raw damage + a guaranteed poison stack, no tactic-change demand. |
